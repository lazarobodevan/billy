...
class LimitPicker extends StatefulWidget {
  ...
  const LimitPicker({super.key, this.limitType, required this.onChanged});
...
class _LimitPickerState extends State<LimitPicker> {
  ...
  @override
  void initState() {
    pickedLimitType = widget.limitType;
    super.initState();
  }
        ...
        if (pickedLimitType == LimitType.TRANSACTION_TYPE) {
          return <DropdownMenuItem<int>>[
            ...
            DropdownMenuItem(
              child: Text("Receita"),
              value:
                  TransactionTypeExtension.toDatabase(TransactionType.INCOME),
            ),
          ];
        }
      }
