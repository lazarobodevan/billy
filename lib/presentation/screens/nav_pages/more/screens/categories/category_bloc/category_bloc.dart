...
import 'package:billy/use_cases/category/create_category_use_case.dart';
  ...
  CategoryBloc(this.repository) : super(LoadingCategoriesState()) {
    ...
    on<SaveCategoryToDatabaseEvent>((event, emit) async {
      ...
      if (category.name.isEmpty) {
        emit(const SaveCategoryToDatabaseErrorState(
      ...
      try {
        ...
        emit(SaveCategoryToDatabaseErrorState(message: e.toString()));
        print(e);
      }
    });
    ...
    on<UpdateCategoryEvent>((event, emit) async {
      emit(LoadingCategoriesState());
      var updatedCategory =
          await _updateCategoryUseCase.execute(event.category);
      categories[categories.indexWhere((cat) => cat.id == event.category.id)] =
          updatedCategory;
      emit(LoadedCategoriesState(categories: categories));
    });
    ...
    on<UpdateCategoryWithSubcategoryEvent>((event, emit) {
      ...
      categories[categories
              .indexWhere((cat) => cat.id == event.subcategory.parentId)]
          .subcategories!
